...
def index(request):
    ...
    last_day_jan = last_day_[0]
    last_day_feb = last_day_[1]
    last_day_mar = last_day_[2]
    last_day_apr = last_day_[3]
    last_day_may = last_day_[4]
    last_day_jun = last_day_[5]
    last_day_jul = last_day_[6]
    last_day_aug = last_day_[7]
    last_day_sep = last_day_[8]
    last_day_oct = last_day_[9]
    last_day_nov = last_day_[10]
    last_day_dec = last_day_[11]
...
def details(request):
    ...
    for i in range(0,len(month)):
        ...
        data.append(InvoiceMonthly.objects.filter(prd_code = prdcode, inv_m_date__year=year, inv_m_date__month=month[i], inv_m_date__day=date[i]).select_related('prd_code','str_code').values('str_code','str_code__str_city','str_code__str_name','prd_code__prd_name','inv_m_save','inv_m_buy','inv_m_return','inv_m_sale','inv_m_stock'))
    # data = InvoiceMonthly.objects.filter(prd_code = prdcode, inv_m_date__year=year).select_related('prd_code','str_code').values('str_code','str_code__str_city','str_code__str_name','prd_code__prd_name').annotate(Sum('inv_m_save'),Sum('inv_m_buy'),Sum('inv_m_return'),Sum('inv_m_sale'),Sum('inv_m_stock'))
