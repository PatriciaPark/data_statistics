    ...
    if request.method == 'POST' and request.FILES['fileInput']:
        ...
        for i in range(0, len(sheetname)):
            ...
            for dbframe in df.itertuples():
                ...
                # 비어있는 날짜 채우기
                # first_day = fromdate_time_obj.replace(day=1)
                # last_day = calendar.monthrange(date_year, date_month)[1]
                # print('*****1st date& last date : ', first_day, last_day)
                
                # btw_date = pd.date_range(first_day,last_day,freq='D')
                # print('*****btw_date : ', btw_date)
